...
func TestUserCanAccessHelpInformation(t *testing.T) {
	...
	if !strings.Contains(helpView, "Commands") || !strings.Contains(helpView, "Additional") {
		t.Error("User should see extended help information")
	}
	
	// Alternative help key should also work
	questionKey := tea.KeyMsg{
		Type:  tea.KeyRunes,
		Runes: []rune("?"),
	}
	appWithHelp2, _ := loadedApp.Update(questionKey)
	helpView2 := appWithHelp2.View()
	
	if !strings.Contains(helpView2, "Commands") {
