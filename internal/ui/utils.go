...
func createTableColumns() []table.Column {
	totalWidth := getTerminalWidth() - 8 // Account for borders, padding, and selection indicators
	
	// Minimum widths to ensure readability
	minPRWidth := 35       // PR title only (no number)
	minAuthorWidth := 14   // Author + Repo
	...
	// Calculate optimal widths for 8 columns (removed Labels, split Activity, added Created)
	prNameWidth := max(minPRWidth, totalWidth * 24 / 100)      // 24% - PR title (no number)
	authorRepoWidth := max(minAuthorWidth, totalWidth * 16 / 100)  // 16% - Author + Repo (increased for better visibility)
	statusWidth := max(minStatusWidth, totalWidth * 12 / 100)   // 12% - Status + CI
	reviewsWidth := max(minReviewsWidth, totalWidth * 10 / 100) // 10% - Review status
	commentsWidth := max(minCommentsWidth, totalWidth * 8 / 100) // 8%  - Comments only
	filesWidth := max(minFilesWidth, totalWidth * 12 / 100)     // 12% - Files only
	createdWidth := max(minCreatedWidth, totalWidth * 9 / 100)  // 9%  - Created time
	updatedWidth := max(minUpdatedWidth, totalWidth * 9 / 100)  // 9%  - Updated time (reduced to make room)
	
	return []table.Column{
		{Title: "Pull Request", Width: prNameWidth},          // PR title only (no number)
		{Title: "Author/Repo", Width: authorRepoWidth},       // Author + repo
...
func createTableRows(prs []*gh.PullRequest) []table.Row {
	...
	for i, pr := range prs {
		...
		// Author + Repo (compact single-line format)
		...
		authorRepo := formatAuthorRepo(author, repoName, columns[1].Width)
		
		// Status + CI (compact single-line format)
		mergeStatus := getPRStatusIndicator(pr)
		ciStatus := "CI:?" // Placeholder for now
...
func getPRReviewIndicator(pr *gh.PullRequest) string {
	...
	if totalReviewers > 0 {
		return fmt.Sprintf("0/%d Rev", totalReviewers) // Shows "0/3 Rev" etc
	}
